...
grafico<-ggplot(df_clean, aes(x = Perfil_Sociabilidad, y = obligaciones, fill = Perfil_Sociabilidad)) +
  geom_boxplot(alpha = 0.7) +
  stat_summary(fun = mean, geom = "point", shape = 18, size = 3, color = "red") +
  labs(
    title = "Compromiso Adaptativo por Perfil de Sociabilidad",
    subtitle = "El punto rojo indica el promedio",
    x = "Perfil de Sociabilidad",
    y = "Compromiso Adaptativo (Tiempo de Obligaciones)",
    fill = "Perfil"
  ) +
  theme_minimal() +
